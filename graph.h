...
#define container_of(ptr, type, member) ((type *) ((char *) (ptr) - offsetof(type, member)))        //to find address of container structure from address of any member of dat structure
...
static inline List* list_last(List *head)           //to get to the last part of list
{
	assert(head);
	while (head->next)
		head = head->next;
...
static inline void list_append(List *head, List *what)  //to append a list to the other mentioned list
{
	List *t = list_last(head);
	t->next = what;
}

static inline void list_delete(List **l, List *what)  //to delete a element from the list
{
	while (*l && *l != what)
		l = &(*l)->next;
	if (!*l)
...
struct node {                   //struct for node in the graph
	/* node id */
	unsigned int id;
	/* node state */
	Status state;
